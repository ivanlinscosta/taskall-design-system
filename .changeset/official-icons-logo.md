...
- Conjunto oficial de 315 ícones (grade 20×20, `currentColor`) gerado de `src/assets/icons` por `pnpm --filter @hive/react generate:icons`, com `iconSearchIndex` (palavras-chave PT/EN).
...
- Novo `Logo` (variantes `full` e `symbol`), temável via `currentColor` e tokens `--hive-logo-accent`/`--hive-logo-on-accent`.
