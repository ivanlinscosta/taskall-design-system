...
describe("Button", () => {
  it("renders with an accessible name", () => {
    renderWithHive(<Button>Salvar</Button>);
    expect(screen.getByRole("button", { name: "Salvar" })).toBeInTheDocument();
  });

  it("forwards the ref to the native button", () => {
    const ref = createRef<HTMLButtonElement>();
    renderWithHive(<Button ref={ref}>Salvar</Button>);
  ...
  it("uses a real button with default type button", () => {
    renderWithHive(<Button>Ok</Button>);
    expect(screen.getByRole("button")).toHaveAttribute("type", "button");
  });

  it("fires onClick when clicked", () => {
    const onClick = vi.fn();
    renderWithHive(<Button onClick={onClick}>Salvar</Button>);
  ...
  it("keeps the label accessible while loading", () => {
    renderWithHive(<Button loading>Salvar</Button>);
    expect(screen.getByRole("button", { name: "Salvar" })).toBeInTheDocument();
  });

  it("renders a decorative left icon hidden from the a11y tree", () => {
    renderWithHive(<Button leftIcon={User}>Perfil</Button>);
